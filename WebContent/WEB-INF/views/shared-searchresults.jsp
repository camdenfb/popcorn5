...
<!-- ARTIST -->
<c:if test="${category == 'Artist '}">
<div class="container2">
 <h3>Search by Artist</h3>
 <h5>${count} Search Results for "${searchstring}"</h5>
<table class="table">
	<c:forEach items="${artresults}" var="art">
	<tr>
	 <td><img src="${context}/images/${art.artistImg}" style="width:80px"></td>
     <td>
     <a href="${pageContext.request.contextPath}/shared/artistpage/${art.artistId}">${art.artistName}</a>
	</td>
	<td>
	 <c:forEach items="${art.topRated()}" var="prod">
	 <c:choose>
  <c:when test="${prod.prod.prodScore() == 0}">
    <img src="${context}/images/blank.png" style="width:20px"> NR
  </c:when>
  <c:when test="${prod.prod.prodScore() < 60}">
    <img src="${context}/images/bad.png" style="width:20px"> ${prod.prod.prodScore()}%
  </c:when>
  <c:otherwise>
    <img src="${context}/images/good.png" style="width:20px"> ${prod.prod.prodScore()}%
  </c:otherwise>
	</c:choose>
	
	   <c:choose>
  <c:when test="${prod.prod.prodType == 'film'}">
  ...
  <c:otherwise>
    <img src="${context}/images/masks.png" style="height:20px">
	  ...
	  <a href="${pageContext.request.contextPath}/shared/prodpage/${prod.prod.prodId}">${prod.prod.prodName} </a><br>
	 </c:forEach>
	</td>
	</tr>
  	</c:forEach>
...
<!-- PROD -->
<c:if test="${category == 'Production '}">
<div class="container2">
 <h3>Search by Production</h3>
 <h5>${count} Search Results for "${searchstring}"</h5>
  
  <ul class="nav nav-tabs">
    <li class="active"><a data-toggle="tab" href="#home">All Productions</a></li>
    <li><a data-toggle="tab" href="#menu1">Film</a></li>
    <li><a data-toggle="tab" href="#menu2">Theater</a></li>
  </ul>
...
<table class="table">
	<c:forEach items="${prodresults}" var="prod">
      <tr>
         <td><img src="${context}/images/${prod.prodPic}" style="width:80px"></td>
  ...
  <c:when test="${prod.prodScore() < 60}">
    <img src="${context}/images/bad.png" style="width:40px"> ${prod.prodScore()}%
  </c:when>
  <c:otherwise>
    <img src="${context}/images/good.png" style="width:40px"> ${prod.prodScore()}%
  </c:otherwise>
	</c:choose>
  ...
  <c:otherwise>
    <img src="${context}/images/masks.png" style="height:30px">
         ...
         <a href="${pageContext.request.contextPath}/shared/prodpage/${prod.prodId}">${prod.prodName}</a>
         </td>
         
         <td>
      <fmt:formatDate pattern="yyyy" value="${prod.prodRelease}"/>
      </td>
...
<table class="table">
	<c:forEach items="${prodresults}" var="prod">
	<c:if test="${prod.prodType == 'theater'}">
      <tr>
         <td><img src="${context}/images/${prod.prodPic}" style="width:80px"></td>
