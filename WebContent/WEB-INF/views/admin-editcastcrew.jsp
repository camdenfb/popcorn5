      ...
      <div id="castcrew" style="height:300px; overflow-y: scroll">
        ...
        <c:forEach items="${artistList}" var="artist">
         <c:set var="match" value="false"/>
		  ...
		  <c:if test="${artist.artistId == artprod.artist.artistId}">
		<c:set var="match" value="true"/>
		<c:set var="role" value="${artprod.role}"/>
		</c:if>
		</c:forEach>
		
		<c:choose>
		<c:when test="${match==true}">
    	
    	 <tr>
			<td><input type="checkbox" name="artists" value="${artist.artistId}" checked/></td>
			<td><img src="${context}/images/${artist.artistImg}" style="width:30px; height:30px"></td>
			<td>${artist.artistName}</td>
			<td><input type="text" id="${artist.artistId}" value="${role}"></td>
			</tr>	
